...
<div class="container-fluid">
  ...
  <?php
    if (isset($_GET['id'])) {
      ...
      $id = $_GET['id'];
      $stmt = $dbh->prepare("SELECT * FROM in_going WHERE id = '$id'");
      $stmt->execute();
...
<div class="row g-3">
    <div class="col-md-6">
      ...
      <input type="text" name="customer_code" class="form-control" id="customer_code" value="<?php echo $fetch['customer_code'];?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" maxlength="3" placeholder="Customer Code" required>
    </div>
    <div class="col-md-6">
      <label class="control-label" for="username">Customer Name:</label>
      <input type="text" name="customer_name" class="form-control" id="customer_name" value="<?php echo $fetch['customer_name'];?>" style="text-transform: capitalize;" placeholder="Customer Name" maxlength="50" required>
    </div>
    <br/>
    <br/>
    <div class="col-md-6">
    ...
    <textarea name="remarks" id="remarks" class="form-control" value="<?php echo $fetch['remarks'];?>" cols="80" rows="10" required><?php echo $fetch['remarks'];?></textarea>  
    </div>
    <br/>
</div>
<br/>
...
if (isset($_POST['in_going_button'])) {
  $id = $_GET['id'];
  $query = "UPDATE `in_going` SET `customer_code` = :customer_code, `customer_name` = :customer_name, `plate_number` = :plate_number, `hawb` = :hawb, `airway_bill` = :airway_bill, 
  `peza_ticket` = :peza_ticket, `trucker` = :trucker, `wafer_run` = :wafer_run, `device_number` = :device_number, `customer_lot_number` = :customer_lot_number, `country_origin` = :country_origin, 
  `plant_site` = :plant_site, `wafer_type` = :wafer_type, `airlines` = :airlines, `driver` = :driver, `container_type` = :container_type, `wafer_size` = :wafer_size, `wafer_number` = :wafer_number,
  `die_qty` = :die_qty, `sawn_date` = :sawn_date, `unit_price` = :unit_price, `p_o` = :p_o, `invoice` = :invoice, `plane_number` = :plane_number, `number_box` = :number_box, `wafer_qty` = :wafer_qty, 
  `customer_info` = :customer_info, `edt_date` = :edt_date, `edt_time` = :edt_time, `eta_date` = :eta_date, `eta_time` = :eta_time, `etc_date` = :etc_date, `etc_time` = :etc_time, `remarks` = :remarks
  WHERE `id` = :id";
